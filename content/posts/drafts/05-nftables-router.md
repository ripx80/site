...
- central router with a public ip address
- clients will connect via open wireguard port to internal network wg0
- clients will communicate with each other via the router (no direct communication)
- use nat masquerade to route traffic to the outsite like a vpn provider does
...
to allow my wireguard clients the connection to the outside world via the network interface 'eth0' and use the masquerading technique to route the client traffic like a vpn provider.
...
(todo): show example nftables ruleset for default nat on a nixos system.

this is bad behavior because traffic is also routed to the internal network without checking the conntrack state first.
...
this creates a **'forward'** and **'forward-allow'** nftable chain.

(todo): example nftable ruleset and explaination

however, i would like to build an advanced router and use my own chain names.
i also think it's always good to know exactly who sets which rules when and to find them in one place in my config.
to achieve exactly that, I use the option **'networking.nftables.ruleSet'**.

this option usually allows you to attach additional rules to the existing ruleset of nftables, which is generated by the various options in nixos.
...
table ip nat {
  chain postrouting {
    ...
    iif "wg0" oif "eth0" masquerade comment "from internal interfaces"
  }
}
```
...
its a good practise to set default policy to **drop**, only defined forwarding accepted.
...
table ip filter {
  ...
  chain forward {
    ...
    iif "wg0" oif "eth0" accept comment "only from wg0 to internet"
    # you need the reverse path for your packages, so only allow related and ethablished packts from the internet
    iif "eth0" oif "wg0" ct state related,established accept comment "allow responses from internet"
  }
}
```

now nftables know which packets should be forwarded and how to translate/nat each packet.
to use the forward like vpn provider, all traffic must be passed through the router.
...
maybe you have notice the **mtu = 1380** setting. the default mtu size is 1500 bytes.
this is due to the overhead of wireguard. but this is a separate story (comming soon).
remember i use a git-crypt encrypted meta.nix file for ip addresses and other confidential stuff.
...
only one forward rule is required

```sh
table ip filter {
    chain forward {
      iif "wg0" oif "wg0" accept comment "allow client communication inside wg0"
    }
}
```

clients can now communicate which each other over the router. this is the simplest configuration.
...
table ip filter {
    chain forward {
      iif "wg0" oif "wg0" jump wg-forward
    }
...
here the rule placed before has a **jump** argument instead of **accept**. nftables will jump to the **wg-forward** chain and look for more detailed instructions.
after processed all rules in 'wg-forward' it will jump back to the position left.
note that when you use a **goto** statement, nftables will not jump back to the prevoius position. if no rule match the default policy kicks in.
...
the vmap on the beginning take care of the back route so response, etablished and related are accepted.
...
- the [difference](https://serverfault.com/questions/985158/what-is-the-difference-between-iifname-and-iif-in-nftables) between iif and ifname, and the problem with non-static ethernet interfaces
